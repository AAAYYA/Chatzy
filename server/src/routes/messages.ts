...
messagesRoute.put('/:id', async (c) => {
    ...
    if (!content || !userId) {
        return c.json({ error: 'Content and userId required' }, 400);
    }

    const existing = await db.select().from(messages).where(eq(messages.id, id));
    if (!existing[0]) return c.json({ error: 'Message not found' }, 404);
    ...
    return c.json({ message: 'Updated', data: updated[0] });
});
